...
describe("computeWeeksKeyNav", () => {
  ...
  it("Shift+PageDown shifts focus +1 year with followFocus", () => {
    ...
    expect(result.action).toBe("move");
    expect((result as Extract<typeof result, { action: "move" }>).date.toString()).toBe("2027-03-15");
  ...
  it("readOnly blocks selection but allows navigation", () => {
    const selectResult = computeWeeksKeyNav({ ...baseInput, key: "Enter", readOnly: true });
    expect(selectResult.action).toBe("none");

    const navResult = computeWeeksKeyNav({ ...baseInput, key: "ArrowRight", readOnly: true });
  ...
  it("Shift+PageUp shifts focus -1 year with followFocus", () => {
    const result = computeWeeksKeyNav({ ...baseInput, key: "PageUp", shiftKey: true });
    expect(result.action).toBe("move");
    expect((result as Extract<typeof result, { action: "move" }>).date.toString()).toBe("2025-03-15");
    expect(result.followFocus).toBe(true);
  });
});
