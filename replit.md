...
- **DatePicker** (`client/src/components/ui/date-picker.tsx`) — A **headless** (zero-styling) compound component built with base-ui internals (`useRender` from `@base-ui/react/use-render`, `mergeProps` from `@base-ui/react/merge-props`). **Internal state uses `Temporal.ZonedDateTime`** (full date-time with timezone), not PlainDate. Sub-components: `DatePicker.Root`, `DatePicker.MonthGrid`, `DatePicker.Week`, `DatePicker.Day`, `DatePicker.DayLabels`, `DatePicker.DayLabel`, `DatePicker.DateString`, `DatePicker.TimeString`, `DatePicker.MonthString`, `DatePicker.PrevMonthButton`, `DatePicker.NextMonthButton`. The old monolithic Header has been decomposed into granular primitives: `MonthString` (formatted month/year display), `DateString` (accepts `locales` + `Intl.DateTimeFormatOptions` matching `Date.toLocaleDateString`), `TimeString` (accepts `locales` + `Intl.DateTimeFormatOptions` matching `Date.toLocaleTimeString`), `PrevMonthButton`, and `NextMonthButton`. Root accepts a `timeZone` prop (defaults to system timezone). When selecting a day, time is preserved from the previous selection (or defaults to midnight). Each sub-component supports the base-ui `render` prop and exposes internal state via `data-` attributes (e.g., `data-selected`, `data-today`, `data-disabled`, `data-outside-month`, `data-focused` on Day; `data-month`, `data-year` on MonthGrid). Day's `focused` state tracks which day currently has keyboard focus. MonthGrid supports a **template-based children API**: pass `<DayLabels>` (containing a single `<DayLabel>` template), `<Week>`, and `<Day>` as templates; MonthGrid clones them for each week/day automatically (injecting `date` and `index` props), removing the need for consumers to call `useDatePicker()` to iterate weeks. `DayLabels` is a container component that clones its single `<DayLabel>` child 7 times (one per weekday). `DayLabel` state exposes `long`, `short`, `narrow` (weekday name formats generated via `Temporal.PlainDate.toLocaleString()` using the locale from Root context) and `dayOfWeek` (0=Sunday through 6=Saturday). Root accepts a `locale` prop (defaults to `"en-US"`) for locale-aware weekday names and other locale-dependent formatting. Root state includes time fields (`selectedHour`, `selectedMinute`, `selectedSecond`) and `timeZone`. The Day component renders as a `<button>` by default (no nested div+button). Uses `@js-temporal/polyfill` for all date calculations. Supports configurable `valueFormat` prop on Root: `"PlainDate"` (default), `"PlainDateTime"`, `"PlainMonthDay"`, `"PlainTime"`, `"PlainYearMonth"`, `"ZonedDateTime"`, `"object"` (plain JS object), or `"Date"` (native JS Date). The `PlainDateObject` type also supports a `timeZone` field. Includes controlled/uncontrolled usage, keyboard navigation, and full ARIA accessibility. Also exports `useDatePicker` hook for consuming context in custom compositions.
- **StyledDatePicker** (`client/src/components/styled-date-picker.tsx`) — A styled composition of the headless DatePicker primitives using shadcn/tailwind design tokens. Uses MonthGrid's **template-based children API**: `<DayLabels>` wrapping a single `<DayLabel>`, plus a `<Week>` containing a single `<Day>` are passed as templates. MonthGrid and DayLabels clone them automatically. No `useDatePicker()` hook needed — pure declarative composition. Includes conditional classes for selected/today/disabled/outside-month/focused states and Lucide icons for navigation arrows.
