...
- **DatePicker** (`client/src/components/ui/date-picker/`) — A **headless** (zero-styling) compound component split into a modular folder structure:
  ...
  - **Styled components** (`styled.tsx`): `StyledPrevMonthButton`, `StyledNextMonthButton`, `StyledMonthYearString`, `StyledDateString`, `StyledTimeString`, `StyledGrid`, `StyledGridHeader`, `StyledGridHeaderCell`, `StyledGridBody`, `StyledWeekTemplate`, `StyledDayCellTemplate`, `StyledDayButton`, `StyledDayTemplate` (composite of cell+button) — each wraps a headless primitive with pre-applied styles. Simple components use `className` merging; complex ones (day button, nav buttons) use `render` props with conditional state-based classes and Lucide icons. All accept additional `className` for extension. Mix-and-match with headless components is supported.
