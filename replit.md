...
- **DatePicker** (`client/src/components/ui/date-picker.tsx`) — A **headless** (zero-styling) compound component built with base-ui internals (`useRender` from `@base-ui/react/use-render`, `mergeProps` from `@base-ui/react/merge-props`). **Internal state uses a discriminated union `DateValueObject`** — a tagged object `{ format: "...", value: ... }` used only internally. Consumers pass **separate `format` and `value` props** with raw Temporal values: `<Root format="ZonedDateTime" value={zdt} onValueChange={(v) => ...}>` where `v` is the raw `Temporal.ZonedDateTime`. The `createDatePicker(format, options?)` factory returns a `TypedDatePicker<F>` where Root's `format` prop is **stripped** — format is locked by the factory and cannot be overridden: `const DP = createDatePicker("ZonedDateTime"); <DP.Root value={zdt} />`. Helper types: `RawValueForFormat<F>` extracts the raw value type (e.g., `RawValueForFormat<"ZonedDateTime">` = `Temporal.ZonedDateTime`), `ValueForFormat<F>` narrows the tagged union (internal use). Supported formats: `"PlainDate"`, `"PlainDateTime"`, `"PlainMonthDay"`, `"PlainTime"`, `"PlainYearMonth"`, `"ZonedDateTime"`, `"object"` (PlainDateObject), `"Date"` (native JS Date). Sub-components: `Root`, `DaysGrid`, `WeekTemplate`, `DayTemplate`, `DayLabels`, `DayLabel`, `DateString`, `TimeString`, `MonthString`, `PrevMonthButton`, `NextMonthButton`. Root accepts `timeZone` (defaults to system), `locale` (defaults to `"en-US"`), `min` and `max` props (same `RawValueForFormat<F>` type as `value`) for date-range bounds — dates outside bounds are disabled and nav buttons show disabled state when navigating would go out of range. Each sub-component supports the base-ui `render` prop and exposes state via `data-` attributes. `Temporal` is resolved at factory time (user-provided > `globalThis.Temporal` > throw). The `_resolvedTemporal` prop is private/internal.
  - **Self-replicating components**: `WeekTemplate`, `DayTemplate`, and `DayLabel` use a "self-replicating" pattern — each reads data from context and renders itself the correct number of times. No parent-side child scanning (`Children.forEach`/`cloneElement`). Each is split into an outer component (maps over data, creates instances) and an inner `*Instance` component (calls `useRender` — hooks can't be called in `.map()`). `WeekTemplate` maps over `weeks` from `useDatePicker()`, providing each week's days via `WeekDataContext`. `DayTemplate` reads days from `WeekDataContext` (if inside `WeekTemplate`) or flattens all weeks (if standalone). `DayLabel` self-replicates 7 times when no `index` prop is provided.
  - **Arbitrary nesting is preserved**: elements inside a self-replicating component repeat per-instance; elements outside render once. `<WeekTemplate><span><DayTemplate /></span></WeekTemplate>` produces the `<span>` wrapper inside each week row.
  - **`DayTemplate` without `WeekTemplate`**: `<DaysGrid><DayTemplate /></DaysGrid>` produces one button per day in the entire grid with no week rows.
  - **`DaysGrid` default children**: `<WeekTemplate><DayTemplate /></WeekTemplate>` when no children are provided.
  - **`DayLabels` default children**: `<DayLabel />` when no children are provided.
- **StyledDatePicker** (`client/src/components/styled-date-picker.tsx`) — A styled composition of the headless DatePicker primitives using shadcn/tailwind design tokens. Accepts a `components` prop of type `DatePickerTyped<F>` (produced by `createDatePicker`), making it generic over the date format. Uses the **self-replicating template API**: `<DayLabels>` wrapping a single `<DayLabel>`, plus `<WeekTemplate>` containing a single `<DayTemplate>` are placed as templates. Each template self-replicates driven by context — no parent-side child scanning needed. No `useDatePicker()` hook needed — pure declarative composition. Includes conditional classes for selected/today/disabled/outside-month/focused states and Lucide icons for navigation arrows.
